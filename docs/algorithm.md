...
###### Расчет первой части
![](https://latex.codecogs.com/svg.latex?C_{f,j}=S_i*K_{f,max}*(1-\frac{\(max\(Rank\)-Rank_j\)}{max\(Rank\)}))

![](https://latex.codecogs.com/svg.latex?K_{f,max}) - процент от среднего значения стейков, который равен первой части комиссии голосования за dapp с наибольшим рейтингом
![](https://latex.codecogs.com/svg.latex?max(Rank)) - масимальное рейтинг во всей системе
