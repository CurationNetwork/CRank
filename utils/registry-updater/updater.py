...
def main(arguments):
    ...
    #     dapps_mix = json.load(json_file)
    #     for d in dapps_mix:
    #         if c1 >= 1:
    #             break
    #         c1 = c1 + 1
    #         dapp = dapps_mix[d]
    #         registry.functions.addDappMetaTemp("" + dapp['name'], 1, json.dumps(dapp)).call()
    
    name = "XYZ"
    metatype = 1
    metadata = json.dumps({'jopa': 1})
    
    # print(dapp_id)
    res = registry.functions.addDappMetaTemp(name, metatype, metadata).call()
    print(repr(res))
    # print(registry.functions.getDAppById.call(dapp_id))
...
def deploy_contract_and_get_instance(web3): # web3 - web3 instance
    ...
    contract_source_code = '''
        ...
        contract RankedRegistry is Ownable /*temporary, for tests*/ {
            ...
            function _getIdByParams(string _name, uint _metatype, string _metadata) internal returns(bytes32) {
                bytes32 id = keccak256(abi.encodePacked(_name, _metatype, _metadata));
                return id;
            }

            function addDappMetaTemp(string _name, uint _metatype, string _metadata) public onlyOwner returns(bytes32) {
                bytes32 id = keccak256(abi.encodePacked(_name, _metatype, _metadata));
